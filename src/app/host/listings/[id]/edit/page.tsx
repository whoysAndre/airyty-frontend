...
export default function EditListingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleDeleteExisting = async (publicId: string) => {
    ...
    try {
      ...
      setServerError(err instanceof Error ? err.message : 'Failed to delete image')
    } finally {
  ...
  if (!listing) {
    return (
      <main className="max-w-2xl mx-auto px-4 sm:px-6 py-8 text-center pt-20">
        ...
        <Link href="/host/listings" className="text-[#ff385c] font-semibold underline mt-4 inline-block">Back to my listings</Link>
      </main>
    )
  }
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5" noValidate>
        ...
        <div>
          <p className="text-sm font-semibold text-[#222] mb-3">
            Photos <span className="text-gray-400 font-normal">({totalImages}/10)</span>
          </p>
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 mb-2">
            {/* Existing Cloudinary images */}
            {existingImages.map((img) => (
              <div key={img.public_id} className="relative aspect-square rounded-xl overflow-hidden bg-gray-100 group">
                <Image src={img.url} alt="" fill className="object-cover" sizes="144px" />
                <button
                  type="button"
